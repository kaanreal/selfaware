...
final class SelfawareMenuPreviewRenderer {
    ...
    static void render(PoseStack poseStack, Font font, int width, int height, int mouseX, int mouseY) {
        ...
        if (player != null) {
            ...
            InventoryScreen.renderEntityInInventory(centerX, bottom - 18, 105, mouseX, mouseY, player);
        } else {
            Component message = Component.nullToEmpty("Join a world to preview");
            GuiComponent.drawCenteredString(poseStack, font, message, (left + right) / 2, bottom / 2, 0xFFBBBBBB);
        }
