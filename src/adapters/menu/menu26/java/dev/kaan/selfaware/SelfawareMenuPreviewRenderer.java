...
final class SelfawareMenuPreviewRenderer {
    ...
    static void render(GuiGraphicsExtractor graphics, Font font, int width, int height, int mouseX, int mouseY) {
        ...
        graphics.text(font, Component.nullToEmpty("Preview"), left + 10, top + 10, 0xFFFFFFFF);

        Minecraft client = Minecraft.getInstance();
        LivingEntity player = client.player;
        if (player != null) {
            InventoryScreen.extractEntityInInventoryFollowsMouse(graphics, left + 12, top + 48, right - 12,
                    bottom - 8, 105, 0.0625F, mouseX, mouseY, player);
        } else {
        ...
        graphics.text(font, name, (left + right - nameWidth - svcWidth) / 2, nameY, 0xFFFFFFFF);
        if (svc != null) {
            graphics.text(font, svc, (left + right + nameWidth - svcWidth) / 2 + 3, nameY, 0xFFAAAAAA);
        }
        if (money != null) {
            graphics.centeredText(font, money, (left + right) / 2, nameY + 15, 0xFFFFFFFF);
        }
    }
}
