...
public final class SelfawareMenuScreen extends Screen {
    ...
    protected void init() {
        ...
        int row = 0;
        nametagButton = addRenderableWidget(new Button(x, y + row++ * 26, buttonWidth, 20, SelfawareMenu.nametagLabel(), button -> {
            SelfawareMenu.toggleNametag();
            nametagButton.setMessage(SelfawareMenu.nametagLabel());
        }));
        if (SelfawareMenu.svcIconsAvailable()) {
            svcIconsButton = addRenderableWidget(new Button(x, y + row++ * 26, buttonWidth, 20,
                    SelfawareMenu.svcIconsLabel(), button -> {
                        SelfawareMenu.toggleSvcIcons();
                        svcIconsButton.setMessage(SelfawareMenu.svcIconsLabel());
                    }));
        }
        serverFormattingButton = addRenderableWidget(new Button(x, y + row++ * 26, buttonWidth, 20,
        ...
        if (SelfawareMenu.donutRankAvailable()) {
            addRenderableWidget(new Button(x, y + row++ * 26, buttonWidth, 20, SelfawareMenu.donutRankLabel(), button -> {
                SelfawareMenu.toggleDonutRank();
                button.setMessage(SelfawareMenu.donutRankLabel());
            }));
        }
        if (SelfawareMenu.donutMoneyAvailable()) {
            donutMoneyButton = addRenderableWidget(new Button(x, y + row++ * 26, buttonWidth, 20,
        ...
        addRenderableWidget(new Button(x, y + row * 26 + 8, buttonWidth, 20, SelfawareMenu.doneLabel(), button -> onClose()));
    }
