...
The opt-in setting `server_formatting_enabled` changes only the local player's name selected by the entity renderer. Off preserves the existing vanilla path. On uses that player's tab-list component when present and nonblank, otherwise their vanilla team-formatted name. `donut_rank_enabled` applies the same selection only when the current server address is DonutSMP. It copies the component so the appearance hook can identify this exact draw without affecting another player, chat, or the tab list. It does not combine tab and team prefixes, which could duplicate a rank.
