...
Selfaware's settings screen uses the same compact vanilla button layout on every version. `/selfaware` and an unbound **Open Selfaware menu** key mapping are built in on every loader. Fabric and Quilt additionally expose it through Mod Menu, which is a required dependency on those loaders. The SVC button appears only when Simple Voice Chat is installed. DonutSMP rank and money appear only on a DonutSMP connection, with money additionally limited to the 1.21.1 and 26.2 implementations. `SelfawareConfig` stores the nametag, Simple Voice Chat icon, server formatting, DonutSMP rank, and Donut money settings in `config/selfaware.properties`, so the choices survive restarts and are shared by every version-specific jar. The SVC toggle is checked before the optional integration renders anything, and the nametag toggle is checked before the local name is selected.
...
The opt-in setting `server_formatting_enabled` changes only the local player's name selected by the entity renderer. Off preserves the existing vanilla path. On uses that player's tab-list component when present and nonblank, otherwise their vanilla team-formatted name. On DonutSMP, `donut_rank_enabled` alone chooses between the raw profile name and the tab-list rank text. This keeps the rank toggle independent while `server_formatting_enabled` controls the detected shadow and background. Components are copied so the appearance hook cannot affect another player, chat, or the tab list.
...
Minecraft still controls name-tag distance, depth behavior, and sneaking visibility. On 1.21.1 and 26.2, the Donut money path reads the compact balance from the tab footer and appends it to the server's existing local text display. Minecraft's display renderer splits the component into two rows and measures one background around both. The server display keeps its own transform, scale, opacity, alignment, and depth behavior. **Server formatting** decides whether its shadow and background are preserved or replaced with the default text-display appearance. The option is exposed only by those version adapters and only while the current server address is `donutsmp.net` or a subdomain. A plugin that offsets text using a transformation rather than mounting or positioning it near the head may not be detected. Nicknames that omit the profile name are deliberately not guessed.

When a server sends a separate text-display nameplate for the local player, Selfaware detects the entity near that player and suppresses the duplicate vanilla nameplate. The 1.21.1 and 26.2 adapters override that display's local render state without changing the networked entity. Turning the main nametag setting off also hides the local server display. Servers using another packet or client renderer remain on the normal path.
