...
A local rerun on 2026-09-17 passed the Fabric, Quilt, and NeoForge 26.2 checks, but Forge 1.20.1 and NeoForge 1.20.2 crashed during resource reload with a missing Mixin `Args$1` class. The same failure reproduces from the pushed baseline `4613dab`, before the menu changes, so it is kept separate from this feature's verification.
...
The visual check stays on Fabric 1.21.1 and the newest Fabric target. Those cover the oldest special renderer and the current renderer while keeping the manual part small. Check `/selfaware`, Mod Menu on Fabric or Quilt, the Controls entry and keybind, button focus and clicks, the SVC button with and without Simple Voice Chat, the server-only DonutSMP buttons, and saved toggles. A new Minecraft or Simple Voice Chat renderer family adds one new smoke target, not another full manual pass.
