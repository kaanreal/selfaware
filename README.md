...
Open the settings screen with `/selfaware`, from Selfaware in Mod Menu on Fabric or Quilt, or by binding **Open Selfaware menu** in Minecraft's Controls screen. Mod Menu is required on Fabric and Quilt. Forge and NeoForge use the built-in command and keybind path. The same simple vanilla buttons are used on every loader and version. Changes apply immediately and are saved in `config/selfaware.properties`. The SVC button is shown only when Simple Voice Chat is installed, and the DonutSMP buttons are shown only while connected to DonutSMP.

Turn on **Server formatting** to use your server-provided tab name, with your normal team-formatted name as the fallback. **DonutSMP rank** applies that same rank formatting only on DonutSMP. On 1.19.4 and newer, Selfaware learns the shadow and background of a nearby text-display nametag and remembers it for that server, so the style stays active after the other player leaves view. The separate **Donut money** option appears while connected to DonutSMP on 1.21.1. It reads your money from the tab footer and renders the name and balance as one Minecraft text display with one shared background. These options default to off. Servers can send different formatting to different viewers, so this is a local approximation, not an exact preview of what everyone else sees.
